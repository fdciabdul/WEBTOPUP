...
<div id="productApp" class="space-y-6">
    ...
    <div class="flex items-center justify-between">
        ...
        <div class="flex items-center gap-2">
            @if($product->status === 'active')
                <span class="bg-green-100 text-green-700 px-3 py-1.5 rounded-full text-xs font-bold">Aktif</span>
            @else
                <span class="bg-slate-200 text-slate-600 px-3 py-1.5 rounded-full text-xs font-bold">Nonaktif</span>
            @endif
        </div>
    </div>
    ...
    <div class="glass-panel rounded-[2rem] overflow-hidden animate-slide-up-fade">
        ...
        <!-- Form Content -->
        <form action="{{ route('admin.products.update', $product->id) }}" method="POST" enctype="multipart/form-data" @submit="prepareSubmit">
            @csrf
            @method('PUT')
                    ...
                    <div class="lg:col-span-4 flex flex-col gap-6">
                        ...
                        <div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 space-y-5">
                            ...
                            <div class="flex items-center justify-between">
                                ...
                                <label class="relative inline-flex items-center cursor-pointer">
                                    <input type="checkbox" name="is_active" value="1" v-model="form.isActive" class="sr-only peer">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 relative z-10">
                                <div class="space-y-2">
                                    ...
                                    <div class="relative">
                                        ...
                                        <select name="category_id" v-model="form.categoryId" required class="input-field pl-11 appearance-none cursor-pointer">
                                ...
                                <div class="space-y-2">
                                    ...
                                    <div class="relative">
                                        ...
                                        <select name="provider" v-model="form.provider" class="input-field pl-11 appearance-none cursor-pointer">
            ...
            <div class="px-8 py-5 bg-white/80 backdrop-blur-md border-t border-slate-100 flex justify-between items-center">
                <div class="flex items-center gap-4">
                    ...
                    <form action="{{ route('admin.products.destroy', $product->id) }}" method="POST" class="inline" onsubmit="return confirm('Yakin ingin menghapus produk ini?')">
                        @csrf
                        @method('DELETE')
                        <button type="submit" class="flex items-center gap-2 px-5 py-2.5 rounded-xl text-red-500 hover:bg-red-50 font-bold text-sm transition-colors">
                ...
                <div class="flex gap-4">
                    ...
                    <button type="submit" class="px-8 py-3 rounded-xl bg-gradient-to-r from-brand-600 to-indigo-600 text-white font-bold shadow-lg shadow-brand-500/30 hover:shadow-brand-500/50 hover:scale-105 active:scale-95 transition-all text-sm flex items-center gap-2">
                        <i class="ri-save-line text-lg"></i> Simpan Perubahan
                    </button>
                </div>
            </div>
        </form>
...
const variantsData = @json($variantsForVue);

createApp({
    data() {
        return {
            activeTab: 'info',
            variantMode: productData.variant_mode || 'simple',
            imagePreview: productData.icon ? (productData.icon.startsWith('http') ? productData.icon : '/storage/' + productData.icon) : null,
            imageUrl: productData.icon && productData.icon.startsWith('http') ? productData.icon : '',
            ...
            form: {
                ...
                provider: productData.provider || 'apigames',
                isBestSeller: productData.is_best_seller || false,
                isFeatured: productData.is_featured || false,
                isActive: productData.status === 'active',
            },

            simpleVariants: productData.variant_mode === 'simple' && variantsData.length > 0
                ? variantsData
                : [{ name: '', stock: 9999999, provider_code: '', download_link: '', prices: { visitor: '', reseller: '', vip: '', vvip: '' } }],

            nestedVariants: productData.variant_mode === 'nested' && variantsData.length > 0
                ? variantsData
                : [{ name: '', items: [{ name: '', stock: 9999999, provider_code: '', download_link: '', prices: { visitor: '', reseller: '', vip: '', vvip: '' } }] }],

            formatInputs: productData.input_fields && productData.input_fields.length > 0
                ? productData.input_fields
                : [{ label: 'User ID', type: 'text', options: '' }],

            customNotes: productData.notes && productData.notes.length > 0
                ? productData.notes.map(n => ({ text: n }))
                : [{ text: '' }]
        }
    },
    methods: {
