...
<div class="max-w-3xl mx-auto space-y-6">
    ...
    <div class="bg-red-50 border border-red-200 rounded-2xl p-4">
        <div class="flex items-center gap-2 text-red-600 font-bold text-sm mb-2">
            <i class="ri-error-warning-fill"></i> Terjadi Kesalahan
        </div>
        ...
        <div class="glass-panel rounded-[2rem] p-8 card-anim animate-slide-up-fade space-y-8">
                ...
                <div class="space-y-5">
                    ...
                    <div>
                        ...
                        <textarea name="description" rows="3"
                                  class="w-full px-4 py-3 rounded-xl bg-white/50 border border-slate-200 focus:border-brand-500 focus:ring-2 focus:ring-brand-500/20 outline-none transition-all text-sm resize-none">{{ old('description', $category->description) }}</textarea>
                    </div>
                    ...
                    <div>
                        ...
                        @php
                            $isRemixIcon = $category->icon && str_starts_with($category->icon, 'ri-');
                            $isImageIcon = $category->icon && !str_starts_with($category->icon, 'ri-');
                        @endphp

                        <!-- Icon Type Selector -->
                        <div class="flex gap-3 mb-4">
                            <label class="flex-1 cursor-pointer">
                                <input type="radio" name="icon_type" value="remix" class="peer hidden" {{ $isRemixIcon || !$category->icon ? 'checked' : '' }} onchange="toggleIconType('remix')">
                            ...
                            <label class="flex-1 cursor-pointer">
                                <input type="radio" name="icon_type" value="image" class="peer hidden" {{ $isImageIcon ? 'checked' : '' }} onchange="toggleIconType('image')">
                                <div class="px-4 py-2 rounded-xl border-2 border-slate-200 text-center font-bold text-xs text-slate-600 peer-checked:bg-brand-600 peer-checked:border-brand-600 peer-checked:text-white transition-all">
                        ...
                        <!-- Remix Icon Input -->
                        <div id="remix-icon-section" class="{{ $isImageIcon ? 'hidden' : '' }}">
                            <div class="flex gap-3 items-end">
                                <div class="w-14 h-14 rounded-xl bg-gradient-to-br from-slate-100 to-white border border-slate-200 flex items-center justify-center text-brand-600 text-2xl shadow-sm" id="icon-preview">
                                    <i id="icon-preview-i" class="{{ $isRemixIcon ? $category->icon : 'ri-layout-grid-fill' }}"></i>
                                </div>
                                <div class="flex-1">
                                    <input type="text" name="icon" id="icon-class-input" value="{{ old('icon', $isRemixIcon ? $category->icon : 'ri-layout-grid-fill') }}"
                        ...
                        <div id="image-upload-section" class="{{ !$isImageIcon ? 'hidden' : '' }}">
                            @if($isImageIcon)
                                <div class="mb-4 inline-block">
                                    <img src="{{ asset('storage/' . $category->icon) }}" alt="{{ $category->name }}" class="w-20 h-20 rounded-xl object-cover shadow-lg">
                                    <p class="text-xs text-slate-400 mt-2">Gambar saat ini</p>
                                </div>
                            @endif
                            <input type="file" name="icon_file" id="icon-input" accept="image/*" class="hidden" onchange="previewImage(event)">
                            <label for="icon-input" id="upload-label" class="flex flex-col items-center justify-center p-6 border-2 border-dashed border-slate-200 rounded-xl cursor-pointer hover:border-brand-500 hover:bg-brand-50/30 transition-all">
                                <i class="ri-image-add-line text-3xl text-slate-400 mb-2"></i>
                                <span class="text-sm font-medium text-slate-600">{{ $isImageIcon ? 'Ganti gambar' : 'Upload gambar' }}</span>
                            </label>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                    ...
                    <div>
                        ...
                        <div class="flex gap-3">
                            <label class="flex-1 cursor-pointer">
                                <input type="radio" name="is_active" value="1" class="peer hidden" {{ old('is_active', $category->is_active) == '1' ? 'checked' : '' }}>
                            ...
                            <label class="flex-1 cursor-pointer">
                                <input type="radio" name="is_active" value="0" class="peer hidden" {{ old('is_active', $category->is_active) == '0' ? 'checked' : '' }}>
            ...
            <div class="grid grid-cols-3 gap-4 p-4 bg-slate-50 rounded-xl">
                <div class="text-center">
                    <p class="text-2xl font-bold text-brand-600">{{ $category->products_count }}</p>
                    <p class="text-xs text-slate-500">Produk</p>
                </div>
                <div class="text-center">
                    <p class="text-sm font-bold text-slate-800">{{ $category->created_at->format('d M Y') }}</p>
                    <p class="text-xs text-slate-500">Dibuat</p>
                </div>
                <div class="text-center">
                    <p class="text-sm font-bold text-slate-800">{{ $category->updated_at->format('d M Y') }}</p>
                    <p class="text-xs text-slate-500">Diupdate</p>
                </div>
            </div>
            ...
            <div class="flex justify-end gap-3 pt-6 border-t border-slate-100">
                ...
                <button type="submit" class="px-6 py-3 bg-gradient-to-r from-brand-600 to-indigo-600 text-white rounded-xl font-bold text-sm shadow-lg shadow-brand-500/30 hover:shadow-brand-500/50 transition-all hover:-translate-y-0.5 active:scale-95">
                    <i class="ri-save-line mr-2"></i> Update Kategori
                </button>
            </div>
        </div>
    </form>
